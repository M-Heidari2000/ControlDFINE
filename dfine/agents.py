...
class IMPCAgent:
    """
    Iterative MPC for locally-linear dynamics.
    ...
    def __call__(self, y, u: Optional[torch.Tensor], explore: bool = False):
        with torch.no_grad():
    ...
    def _build_lindx(self, x0: torch.Tensor) -> LinDx:
        ...
        for t in range(self.planning_horizon):
            A, B, _, _, _ = self.dynamics_model.get_dynamics(x)
            F_list.append(torch.cat([A, B], dim=2))
